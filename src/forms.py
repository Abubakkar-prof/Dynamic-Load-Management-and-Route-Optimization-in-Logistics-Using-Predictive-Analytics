...
from wtforms import StringField, IntegerField, FloatField, SelectField, TextAreaField, DateField, PasswordField, BooleanField
...
class OrderForm(FlaskForm):
    ...
    weight_kg = FloatField('Weight (kg)', validators=[DataRequired(), NumberRange(min=0.1, max=10000)])
    volume_m3 = FloatField('Volume (m³)', validators=[Optional(), NumberRange(min=0, max=100)])
    ...
    priority = IntegerField('Priority (1-4)', validators=[DataRequired(), NumberRange(min=1, max=4)])
    deadline_hour = IntegerField('Deadline Hour (0-23)', validators=[Optional(), NumberRange(min=0, max=23)])
...
class VehicleForm(FlaskForm):
    ...
    capacity_kg = FloatField('Capacity (kg)', validators=[DataRequired(), NumberRange(min=1, max=50000)])
    capacity_vol = FloatField('Volume Capacity (m³)', validators=[Optional(), NumberRange(min=0, max=200)])
