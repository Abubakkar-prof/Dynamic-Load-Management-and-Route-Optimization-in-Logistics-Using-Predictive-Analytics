...
class LogisticsOptimizer:
    ...
    def optimize_routes(self):
        ...
        locations = [self.depot] + list(zip(self.orders['latitude'], self.orders['longitude']))
        demands_kg = [0] + list(self.orders['weight_kg'].astype(int)) # 0 for depot
        demands_vol = [0] + list(self.orders['volume_m3']) # Checking Weight primarily for CVRP here
